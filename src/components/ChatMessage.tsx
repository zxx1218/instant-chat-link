...
export function ChatMessage({
  ...
  const label = getUserLabel(senderId, isSelf);

  return (
    <div
      className={cn(
        "flex message-enter gap-2 items-end",
        isSelf ? "justify-end" : "justify-start"
      )}
    >
      {!isSelf && (
        <div className="w-8 shrink-0">
          {showSender && <UserAvatarBadge userId={senderId} size="sm" title={label} />}
        </div>
      )}

      <div className={cn("max-w-[80%] sm:max-w-[70%] flex flex-col", isSelf ? "items-end" : "items-start")}>
        {showSender && (
          <span
            className={cn(
              "text-[10px] mb-0.5 px-1 text-muted-foreground",
        ...
        <div
          className={cn(
            "px-3 py-2 sm:px-4 sm:py-2.5 rounded-2xl shadow-sm",
            isSelf
